...
void WebSocketServer::listen() {
    ...
    FishinoClient cli = m_server.available();
    if( !cli )
        return;
...
WebSocket::WebSocket( WebSocketServer *server, FishinoClient cli ) :
    m_server(server),
    client(cli)
{
    if( doHandshake() )
...
bool WebSocket::doHandshake() {
    ...
    while ((bite = client.read()) != -1) {
        ...
        if (counter > 2 && (bite == '\n' || counter >= 127)) { // EOL got, or too long header. temp should now contain a header string
            ...
            if (!hasUpgrade && strstr(temp, PSTR("Upgrade: "))) {
                // OK, it's a websockets handshake for sure
                hasUpgrade = true;	
            } else if (!hasConnection && strstr(temp, PSTR("Connection: "))) {
                hasConnection = true;
            } else if (!hasHost && strstr(temp, PSTR("Host: "))) {
                hasHost = true;
            } else if (!hasKey && strstr(temp, PSTR("Sec-WebSocket-Key: "))) {
                hasKey = true;
                strtok(temp, " ");
                strcpy(key, strtok(NULL, " "));
            } else if (!isSupportedVersion && strstr(temp, PSTR("Sec-WebSocket-Version: ")) && strstr(temp, PSTR("13"))) {
	...
	char buf[132];
        snprintf( buf, sizeof(buf), PSTR("HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Accept: %s\r\n\r\n"), temp );
	client.print( buf );
    } else {
